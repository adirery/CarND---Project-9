...
size_t N = 20;
double dt = 0.05;
const int latency = 2; //2 * 0.05 = 100ms
...
// This is the reference speed (can be tuned)
double ref_v = 50;
...
public:
	...
	void operator()(ADvector& fg, const ADvector& vars) {
		...
		for (int t = 0; t < N; t++) {
			fg[0] += CppAD::pow(vars[cte_start + t], 2);				// taking into account cross-track-error
			fg[0] += CppAD::pow(vars[epsi_start + t], 2);				// taking into account psi error
			fg[0] += CppAD::pow(vars[v_start + t] - ref_v, 2);	// taking into account speed error
		}

		// Minimize the use of actuators.
		for (int t = 0; t < N - 1; t++) {
			fg[0] += CppAD::pow(vars[delta_start + t], 2);			// taking into account steering actuator
			fg[0] += CppAD::pow(vars[a_start + t], 2);					// taking into account acceleration actuator
		}

		// Minimize the value gap between sequential actuations.
		for (int t = 0; t < N - 2; t++) {
			fg[0] += 600 * CppAD::pow(vars[delta_start + t + 1] - vars[delta_start + t], 2);	// taking into account steering actuator "smoothing"
			fg[0] += CppAD::pow(vars[a_start + t + 1] - vars[a_start + t], 2);					// taking into account acceleration actuator "smoothing"
		}
		...
		for (int t = 0; t < N - 1; t++) {
			...
			// The equations for the physical model:
...
Result MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
	...
	for (unsigned int i = 0; i < N - 1; i++) {
		result.steering.push_back(solution.x[delta_start + i]);
