      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          ...
          double v = j[1]["speed"];
					...
					// Transform the coordinates to the car's coordinate system
					...
					// Calculate coefficients for 3rd degree polynomial through the waypoints
					auto coeffs = polyfit(Ptsx, Ptsy, 3);

					// The cross track error is calculated by evaluating at polynomial at x, f(x) and subtracting y.
					// since car is always at x=0 the formula simplifies
					double cte = polyeval(coeffs, 0);

					// Due to the sign starting at 0, the orientation error is -f'(x).
					// derivative of coeffs[0] + coeffs[1] * x -> coeffs[1]
					// since psi is always 0 formula simplifies
					...
					// compute the optimal trajectory, set steering, throttling, X-path, Y-path   
					auto result = mpc.Solve(state, coeffs);

					auto steer_value = result.steering.at(3); // picking 2 due to latency, otherwise would be 1
					auto acceleration_value = result.acceleration.at(3); // picking 2 due to latency, otherwise would be 1
					
					// setting steering & throttling value for mpc smoothing
					mpc.delta_prev = steer_value;
					mpc.a_prev = acceleration_value;

          json msgJson;
          // NOTE: Remember to divide by deg2rad(25) before you send the steering value back.
          // Otherwise the values will be in between [-deg2rad(25), deg2rad(25)] instead of [-1, 1].
          msgJson["steering_angle"] = -steer_value / 0.436332;
          msgJson["throttle"] = acceleration_value;
